...
public class AssetDao extends BaseDao implements IAssetDao {
	...
	public List<Transbill> getAssetTransbillList(Integer assetid) {
		...
		return this
				.getHibernateTemplate()
				.find("select transbill from Transbill as transbill where transbill.asset.assetid=? ",
						o);
	...
	public List<Zctransdetail> getZctransdetailList(Integer userid) {
		...
		return this
				.getHibernateTemplate()
				.find("select zctransdetail from Zctransdetail as zctransdetail where zctransdetail.userinformation.userid=? and zctransdetail.jsresult=?",
						o);
	...
	public List<Asset> getAssetList(String assetname, String assetcoding) {
		List<Object> condList = new ArrayList<Object>();
		condList.add(3);
		String aql = "select asset from Asset as asset where asset.zkstate!=?";
		if (assetname != null && !assetname.equals("")) {
			condList.add("%" + assetname + "%");
			aql = aql + " and asset.assetname like ?";
		}
		if (assetcoding != null &&!assetcoding.equals("")) {
			condList.add(assetcoding);
			aql = aql + " and asset.assetcoding=?";
		}
		Object o[] = condList.toArray();
		System.out.println("assetname:"+assetname);
		System.out.println("assetcoding:"+assetcoding);
		System.out.println("sql:"+aql);
		return this.getHibernateTemplate().find(aql, o);
	}
